...
public class GlobalNavAnimation implements AnimationListener 
{
    ...
    private Animation menuContainerSlideIn, menuContainerSlideOut, sectionContainerSlideIn, sectionContainerSlideOut;

    private static int sectionContainerLayoutWidth, sectionContainerLayoutHeight;
    ...
    public void initializeFilterAnimations(RelativeLayout menuContainer)
    {
        ...
        menuContainerSlideIn = AnimationUtils.loadAnimation(mContext, R.anim.filter_slide_in);

        menuContainerSlideOut = AnimationUtils.loadAnimation(mContext, R.anim.filter_slide_out);    
    ...
    {       
        ...
        sectionContainerSlideIn = AnimationUtils.loadAnimation(mContext, R.anim.other_slide_in);
        sectionContainerSlideIn.setAnimationListener(this);

        sectionContainerSlideOut = AnimationUtils.loadAnimation(mContext, R.anim.other_slide_out);
        sectionContainerSlideOut.setAnimationListener(this);
    }

    public void toggleSliding()
    {
        if(isSectionContainerSlideOut) //check if findLayout is already slided out so get so animate it back to initial position
        {
    ...
    {
        if(isSectionContainerSlideOut) //Now here we will actually move our view to the new position,because animations just move the pixels not the view
        {
            RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(sectionContainerLayoutWidth, sectionContainerLayoutHeight);
